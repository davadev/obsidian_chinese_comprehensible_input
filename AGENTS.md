...
- **`@codemirror/lang-markdown` is BUNDLED into `main.js`, not external** (unlike `@codemirror/state`/`view`/`language`, which are). Bumping it changes shipped bytes and carries real risk: `markdownRendering.ts` holds workarounds keyed to its parsing behaviour. It ships on a release of its own with a focused reading-view test — never bundled with other changes.
